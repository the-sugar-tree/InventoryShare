...
public class Inventory_1_19_R3 implements IInventory {
    ...
    public void savePlayerInventory(@NotNull Player p) {
        ...
        EntityPlayer entityPlayer = ((CraftPlayer) p).getHandle();
        try {
            setField(pinv, "i", entityPlayer.fJ().i);
            setField(pinv, "j", entityPlayer.fJ().j);
            setField(pinv, "k", entityPlayer.fJ().k);
            setField(pinv, "o", ImmutableList.of(entityPlayer.fJ().i, entityPlayer.fJ().j, entityPlayer.fJ().k));
        } catch (Exception e) {
            e.printStackTrace();
        }
